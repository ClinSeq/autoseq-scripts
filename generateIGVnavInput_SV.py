...
def parse_svaba(input_vcf, SDID, output, vcftype):
    ...
    """
    header = "echo \"CHROM\tSTART\tEND\tSDID\tSVTYPE\tALT\tSUPPORT_normal\tSUPPORT_tumor\tDPnormal\tDPtumor\tGENES\"" + " > "+ output +  "_" + vcftype + "_svaba.mut"
    svaba_cmd = "vawk '{print $1, $2, $2+1, \"" + SDID + "_svaba\", \"BND\", $5, S$*$AD, S$*$DP}' " + \
                " " + input_vcf + " >> " + output + "_" + vcftype + "_svaba.mut"

    cmd = "awk '{print $1, $2, $3, $4, $5," + sv_caller + vcftype + "}' "  + output + "_" + vcftype + "_svaba.mut >> " + output_dir + "/annotate_combined_sv.txt"
    subprocess.call(" && ".join([header, svaba_cmd, cmd]), shell=True)
...
def parse_svict(input_vcf, SDID, output, vcftype):
    ...
    """
    header8 = "echo \"CHROM\tSTART\tEND\tSDID\tSVTYPE\tALT\tSUPPORT_READS\tGENES\"" + " > "+ output + "_svict_SR8.mut"
    header12 = "echo \"CHROM\tSTART\tEND\tSDID\tSVTYPE\tALT\tSUPPORT_READS\tGENES\"" + " > "+ output + "_svict_SR12.mut"
    ...
    cmd = "awk '{print $1, $2, $3, $4, $5," + sv_caller + vcftype + "}' "   +output + "_svict_SR8.mut" + ">> " + output_dir + "/annotate_combined_sv.txt"
    subprocess.call(" && ".join([header8, header12, sup_8, sup_12, cmd]), shell=True)
...
def parse_lumpy(input_vcf, SDID, output):
    ...
    """
    header1k_sup_50 = "echo \"CHROM\tSTART\tEND\tSDID\tSVTYPE\tALT\tSUPPORT_READS\tGENES\"" + " > "+ output + "_lumpy_len1k_SU50.mut"
    header500_sup_24 = "echo \"CHROM\tSTART\tEND\tSDID\tSVTYPE\tALT\tSUPPORT_READS\tGENES\"" + " > "+ output + "_lumpy_len500_SU24.mut"
    ...
    len1k_sup_50 = "vawk '{if ((I$SVLEN>1000 || I$SVLEN<-1000) && $1 != \"hs37d5\" && $1 !~ \"GL\" && $5 !~ \"hs37d5\" && I$SU>50 && I$SVTYPE ~ \"BND\") print $1, $2, $2+1, \""+ SDID +"_lumpy\", I$SVTYPE, $5, \"NA\", I$SU ;"  + \
                " else if ((I$SVLEN>1000 || I$SVLEN<-1000) && $1 != \"hs37d5\" && $1 !~ \"GL\" && $5 !~ \"hs37d5\" && I$SU>50 && I$SVTYPE !~ \"BND\") print $1, $2, I$END, \""+ SDID +"_lumpy\", I$SVTYPE, $5, I$SU}' " + input_vcf + \
    ...
    cmd = "awk '{print $1, $2, $3, $4, $5," + sv_caller + vcftype + "}' " + output + "_lumpy_len500_SU24.mut" +  ">> " + output_dir + "/annotate_combined_sv.txt"
    subprocess.call(" && ".join([header1k_sup_50, header500_sup_24, len1k_sup_50, len500_sup_24, cmd]), shell=True)
...
parser.add_argument('--annotBed', required=True, help="UCSC hg19 genes bed file with chrom, start, end and genesymbol")
...
if not os.path.exists(output_dir + "/annotate_combined_sv.txt"):
    header2 = "echo \"CHROM\tSTART\tEND\tSVTYPE\tTOOL\tSAMPLE\tGENES" + ">>" + output_dir + "/annotate_combined_sv.txt"
    subprocess.call(header2, shell=True)



if sv_caller == 'svict':
    parse_svict(vcf, sdid, output, vcftype)
    ...
    parse_svcaller(gtf, sdid, output)
